...
class Spot2DReward(RewardModel):
    def __init__(self, goal_pos: np.ndarray | jnp.ndarray | None = None, action_cost=0.0): # TODO check input is 2D
...
class Spot2DEnv(SpotGym):
    ...
    def __init__(self,
        ...
        super().__init__(config, cmd_freq, monitor_freq, log_dir=log_dir, session=session, log_str=True)
    ...
    def get_obs_from_state(self, state: SpotState) -> np.ndarray:
        ...
        """
        theta0 = self.body_start_frame.angle
        return Spot2DEnv.get_obs_from_state_goal(state, self.goal_pos, theta0)

    @staticmethod
    def get_obs_from_state_goal(state: SpotState, goal: np.ndarray, angle: float) -> np.ndarray:
        origin = goal
        theta0 = angle
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None) -> Tuple[Any, dict]:
        ...
        if goal is None:
            self.goal_pos = self.body_start_frame.inverse(np.array([self.config.start_x, self.config.start_y]))
        else:
            self.goal_pos = self.body_start_frame.inverse(np.array(goal[:2])) # convert to vision frame
